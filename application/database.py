...
class DataBase:
    ...
    def _create_table(self):
        ...
        query = f"""CREATE TABLE IF NOT EXISTS {PLAYLIST_TABLE}
                    (name TEXT, content TEXT, dummy TEXT, time Date, id INTEGER PRIMARY KEY AUTOINCREMENT)"""
    ...
    def get_all_messages(self, limit=100, name=None):
        ...
        for r in sorted(result, key=lambda x: x[3], reverse=True)[:limit]:
            name, content, dummy, date, id = r
            data = {"name":name, "message":content, "dummy":dummy, "time":str(date), "id":id}
            results.append(data)
    ...
    def save_message(self, name, msg, dum):
        """
        ...
        self.cursor.execute(query, (name, msg, dum, datetime.now().strftime("%d-%b-%y %H:%M"), None))
        self.conn.commit()

    def delete_message(self, id):
        query = f"DELETE FROM {PLAYLIST_TABLE} WHERE id=?"
